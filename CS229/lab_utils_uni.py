...
def plt_intuition(x_train, y_train):
    ...
    def func(w=150):
        ...
        ax[1].scatter(
            w, cur_cost, s=100, color=dldarkred, zorder=10, label=f"cost at w={w}"
        )
        ax[1].hlines(
            cur_cost, ax[1].get_xlim()[0], w, lw=4, color=dlpurple, ls="dotted"
        )
        ax[1].vlines(
            w, ax[1].get_ylim()[0], cur_cost, lw=4, color=dlpurple, ls="dotted"
        )
...
def plt_stationary(x_train, y_train):
    ...
    w_range = np.array([200 - 300.0, 200 + 300])
    b_range = np.array([50 - 300.0, 50 + 300])
    ...
    ### plot contour ###
    CS = ax[1].contour(
        tmp_w, tmp_b, np.log(z), levels=12, linewidths=2, alpha=0.7, colors=dlcolors
    )
    ...
    cscat = ax[1].scatter(
        w0, b, s=100, color=dlblue, zorder=10, label="cost with \ncurrent w,b"
    )
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            b = self.ax[1].hlines(
                bs, self.ax[1].get_xlim()[0], ws, lw=4, color=dlpurple, ls="dotted"
            )
            c = self.ax[1].vlines(
                ws, self.ax[1].get_ylim()[0], bs, lw=4, color=dlpurple, ls="dotted"
            )
            d = self.ax[1].annotate(
                f"Cost: {cst:.0f}",
                xy=(ws, bs),
                xytext=(4, 4),
                textcoords="offset points",
